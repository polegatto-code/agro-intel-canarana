...
| **Integração Fase 7** | Os novos motores (Agronômico, Revalidação, Consenso, Mercado) foram criados mas não integrados ao fluxo principal. | Funcionalidades novas não estão em uso no sistema real. | Crítica | Realizar a integração no `scheduler.ts` conforme plano da Fase 8. |
| **Stubs de Clima** | 5 das 6 fontes de clima no `weatherConsensus.ts` são stubs. | Baixa redundância real de dados climáticos (dependência de OpenWeather). | Média | Implementar adaptadores reais para WeatherAPI e INMET. |
| **`newsCollector.ts` vs `marketIntelligence.ts`** | Duplicidade conceitual entre o coletor antigo e o novo motor de inteligência. | Confusão arquitetural. | Alta | Unificar os serviços, usando o `marketIntelligence.ts` como o cérebro interpretador. |
